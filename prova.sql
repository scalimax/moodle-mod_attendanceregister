SELECT u.id FROM mdl_user u JOIN 
    ...
    WHERE u.lastaccess + (30 * 60) < 1701094029
        AND (NOT EXISTS (SELECT * FROM mdl_attendanceregister_session as3 WHERE as3.userid = u.id AND as3.register = 29 AND as3.onlinesess = 1)
            ...
            OR EXISTS (SELECT * FROM mdl_attendanceregister_aggregate aa2, mdl_log l2
...
(NOT EXISTS (SELECT * FROM mdl_attendanceregister_session as3 WHERE as3.userid = 2432 AND as3.register = 29 AND as3.onlinesess = 1)
            OR NOT EXISTS (SELECT * FROM mdl_attendanceregister_aggregate aa4 WHERE aa4.userid=2432 AND aa4.register=29 AND aa4.grandtotal = 1)
            OR EXISTS (SELECT * FROM mdl_attendanceregister_aggregate aa2, mdl_logstore_standard_log l2
                                    WHERE aa2.userid = 2432 AND aa2.register = 29
